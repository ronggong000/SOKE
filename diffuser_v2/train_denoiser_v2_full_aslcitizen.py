...
_DEFAULT_ASLC_SIDE_WEIGHT = (
    "/home/smuk0019/ar85_scratch2/singyu/aslcitizen/"
    "experiments_with_vel_arg/softweight_time_only/tcn_tpp__both/gradcam"
)
...
def _normalize_sample_id_for_merge(value):
...
def _resolve_full_train_csv_paths(opt):
    user_paths = [str(p).strip() for p in getattr(opt, "full_train_csv_paths", []) if str(p).strip()]
    if user_paths:
        paths = user_paths
    else:
        dirs = []
        train_dir = os.path.dirname(str(getattr(opt, "train_csv_path", "") or "").strip())
        val_dir = os.path.dirname(str(getattr(opt, "val_csv_path", "") or "").strip())
    ...
    if missing:
        raise FileNotFoundError(
            "Missing split csv for full ASLcitizen train merge:\n"
            + "\n".join(missing)
            + "\nPass explicit --full_train_csv_paths."
        )
    return paths


def build_full_aslcitizen_train_csv(csv_paths, out_csv_path):
    dfs = []
    for p in csv_paths:
        sep = _infer_sep(p)
    ...
    col_name = _col_lookup(
        merged,
        ["SENTENCE_NAME", "sentence_name", "name", "SAMPLE_ID", "sample_id", "Video file", "video file", "video_file"],
    )
    merged["__sid_norm__"] = merged[col_name].apply(_normalize_sample_id_for_merge)
    before = len(merged)
    merged = merged.drop_duplicates(subset=["__sid_norm__"], keep="first")
    merged = merged.drop(columns=["__sid_norm__"])
    merged = merged.reset_index(drop=True)

    merged.to_csv(out_csv_path, index=False)
    print(
        f"[FULL-ASLC] merged split csv -> {out_csv_path} | "
        f"rows: {before} -> dedup: {len(merged)} | sources={len(csv_paths)}"
    )
    return out_csv_path
...
def save_runtime_opt(opt):
    ...
    with open(file_name, "wt", encoding="utf-8") as opt_file:
        ...
        for k, v in sorted(args.items()):
            opt_file.write("%s: %s\n" % (str(k), str(v)))
        opt_file.write("-------------- End ----------------\n")
    print(f"[FULL-ASLC] runtime options saved to: {file_name}")
...
if __name__ == '__main__':
    opt = arg_parse(True)
    opt.full_aslcitizen_mode = True

    full_csv_paths = _resolve_full_train_csv_paths(opt)
    merged_train_csv = pjoin(opt.model_dir, "full_aslcitizen_train_merged.csv")
    opt.train_csv_path = build_full_aslcitizen_train_csv(full_csv_paths, merged_train_csv)

    if bool(getattr(opt, "enable_custom_weight", False)):
        if not str(getattr(opt, "custom_weight_dir", "") or "").strip():
            opt.custom_weight_dir = _DEFAULT_ASLC_SIDE_WEIGHT
            print(f"[CustomWeight] custom_weight_dir not set, using default: {opt.custom_weight_dir}")
        if not bool(getattr(opt, "custom_weight_precheck", False)):
            opt.custom_weight_precheck = True
            print("[CustomWeight] forcing custom_weight_precheck=True for full ASLcitizen training.")
        cw_dir_abs = os.path.abspath(str(opt.custom_weight_dir))
        train_dir_abs = os.path.abspath(str(opt.train_data_dir))
        if cw_dir_abs == train_dir_abs:
            raise ValueError(
                "custom_weight_dir is a sidecar weight directory and cannot be used as train_data_dir."
            )
