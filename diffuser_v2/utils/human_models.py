...
human_model_path = '/home/smuk0019/ar85_scratch2/singyu/unet/deps/smpl_models'
class SMPLX(object):
    def __init__(self):
        ...
        with open(osp.join(human_model_path, 'smplx', 'SMPLX_to_J14.pkl'), 'rb') as f:
            self.j14_regressor = torch.from_numpy(pickle.load(f, encoding='latin1')).float()
        ...
        self.J_regressor = self.layer['neutral'].J_regressor
        ...
        self.joint_part = \
        {'upper_body': list(range(self.joints_name.index('Neck'), self.joints_name.index('R_Wrist')+1)) + list(range(self.joints_name.index('L_Ear'), self.joints_name.index('Nose')+1)),
        'lhand': range(self.joints_name.index('L_Thumb_1'), self.joints_name.index('L_Pinky_4')+1),
        'rhand': range(self.joints_name.index('R_Thumb_1'), self.joints_name.index('R_Pinky_4')+1),
        'hand': range(self.joints_name.index('L_Thumb_1'), self.joints_name.index('R_Pinky_4')+1),
        'face': range(self.joints_name.index('Face_1'), self.joints_name.index('Face_72')+1)}
        ...
        self.joint_part2idx = {}
        for k,v in self.joint_part.items():
            self.joint_part2idx[k] = list(self.joint_idx[v])

        with open(osp.join(human_model_path, 'smplx_vert_segmentation.json'), 'r') as f:
            self.vertex_idx = json.load(f)
        vertex_groups = {
            'upper_body': ['leftArm', 'leftForeArm', 'leftShoulder', 'neck', 'rightArm', 'rightForeArm', 'rightShoulder'],
            'body': ['rightUpLeg', 'leftArm', 'head', 'leftEye', 'rightEye', 'leftLeg', 'leftToeBase', 'leftFoot', 'spine1', 'spine2', 'leftShoulder', 'rightShoulder', 'rightFoot', 'rightArm', 'rightLeg', 'leftForeArm', 'rightForeArm', 'rightToeBase', 'spine', 'leftUpLeg', 'eyeballs', 'hips'],
            'lhand': ['leftHand', 'leftHandIndex1'],
            'rhand': ['rightHand', 'rightHandIndex1'],
            'hand': ['leftHand', 'leftHandIndex1', 'rightHand', 'rightHandIndex1'],
            'face': ['eyeballs', 'leftEye', 'rightEye', 'head']
        }
        self.vertex_part2idx = {
            'upper_body': [],
            'body': [],
            'lhand': [],
            'rhand': [],
        ...
        for k,v in vertex_groups.items():
            for p in v:
                self.vertex_part2idx[k].extend(self.vertex_idx[p])
