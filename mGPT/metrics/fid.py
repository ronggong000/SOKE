...
def smpl_fid(pred, gt):
    m_pred = np.mean(pred, axis=0)
    s_pred = np.cov(pred, rowvar=False)

    m_gt = np.mean(gt, axis=0)
    s_gt = np.cov(gt, rowvar=False)

    fid_value = calculate_frechet_distance(m_pred, s_pred, m_gt, s_gt)
    return fid_value


def main():
    pred = np.random.rand(10, 20)
    gt = np.random.rand(10, 20)
    fid = smpl_fid(pred, gt)
    print(fid)
