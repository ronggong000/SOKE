...
def _make_component_indices(joints_num: int, per_joint_dim: int = 12):
    pose_idx = []
    ric_idx = []
    rot_vel_idx = []
    ric_vel_idx = []
    for j in range(joints_num):
        base = j * per_joint_dim
        pose_idx.extend([base + 0, base + 1, base + 2])
        ric_idx.extend([base + 3, base + 4, base + 5])
        rot_vel_idx.extend([base + 6, base + 7, base + 8])
        ric_vel_idx.extend([base + 9, base + 10, base + 11])
    return pose_idx, ric_idx, rot_vel_idx, ric_vel_idx
...
def arg_parse(is_train: bool = False):
    ...
    # basic setup
    parser.add_argument("--name", type=str, default="vae12_hier_12d")
    ...
    parser.add_argument(
        "--data_format",
        type=str,
        default="motion_dataset_rod3_fixed_length_dk",
    ...
    parser.add_argument("--lambda_kl", type=float, default=0.02)
    parser.add_argument("--lambda_mesh", type=float, default=1.0)
    parser.add_argument("--lambda_recon_pose", type=float, default=1.0)
    parser.add_argument("--lambda_recon_ric", type=float, default=1.0)
    parser.add_argument("--lambda_recon_rot_vel", type=float, default=1.0)
    parser.add_argument("--lambda_recon_ric_vel", type=float, default=1.0)
    ...
    # 12D fixed feature setup
    opt.per_joint_dim = 12
    opt.joint_feature_dims = [opt.per_joint_dim] * opt.joints_num
    opt.input_dim = sum(opt.joint_feature_dims)
    opt.xyz = False

    pose_idx, ric_idx, rot_vel_idx, ric_vel_idx = _make_component_indices(opt.joints_num, opt.per_joint_dim)
    opt.pose_feature_indices = pose_idx
    opt.ric_feature_indices = ric_idx
    opt.rot_vel_feature_indices = rot_vel_idx
    opt.ric_vel_feature_indices = ric_vel_idx
