...
from mymodel.tools.rag import _load_wlasl_map, _normalize_gloss_sentence,_lookup_wlasl_entry,_entry_tokens_to_matrix,build_blueprint_batch
...
CONFIG = {
    ...
    "text_source": "text",
    # 保存目录
    "save_dir": "checkpoints/maskgit_v1_double_emb_24depth_rag_nogroup_vloss",
    ...
    "mask_ratio_min_schedule": {"type":"cosine","start":0.15,"end":0.55,"warmup_epochs":50},
    "p_textonly_train_schedule": {"type":"linear","start":0.00,"end":0.15,"warmup_epochs":50},
    ...
    #   stage=3: all slots
    # resume
    "resume_path": "",        # path to a checkpoint .tar (optional)
    "resume_last": False,     # if True and resume_path is empty, try <save_dir>/maskgit_last.tar
    "resume_optim": True,     # restore optimizer/scaler when resuming
    ...
    "time_shift_aug_max": 1,#1  # e.g., 1/2/4. With VQ compress=4, shift=1 token ~= 4 frames.

    # shift-tolerant CE (token-level). 0 disables. Recommend 1-2.
    "shift_tolerant_max": 1,#1
    ...
    "stage_mode": "auto",
    "stage": 1,
    "stage_schedule": [(0, 1), (20, 2)],   # stage1 至少 20 epoch
    ...
    "use_blueprint": True,
    "cfg_drop_prob": 0.15,   # CFG: 训练时丢条件概率
    "tie_groups": False,
    "vertex_loss_weight": 0.1,
    "vertex_loss_prob": 0.10,          # 每 10 步算 1 步左右（随机）
    "vertex_loss_max_frames": 128,     # 每次最多抽 128 帧
    "vertex_soft_temp": 1.0,
}
...
def build_slot2q_and_sizes_from_meta(meta: dict):
    """
    Returns:
      slot2q_idx: List[int] length K
      q_idx_to_size: Dict[int,int]  (q_idx -> codebook_size)
    Works when meta has either:
      - explicit slot2q_idx / q_idx_to_size
      - or only "groups": [{"ids":[...], "q_idx":..., "codebook_size":...}, ...]
    """
    slots = list(meta.get("slots", []))
    K = int(meta.get("K", len(slots)))

    # Case A: already provided
    if "slot2q_idx" in meta and "q_idx_to_size" in meta:
        slot2q_idx = list(meta["slot2q_idx"])
        q_idx_to_size = {int(k): int(v) for k, v in dict(meta["q_idx_to_size"]).items()}
        if len(slot2q_idx) != K:
            raise ValueError(f"[meta] slot2q_idx len mismatch: {len(slot2q_idx)} vs K={K}")
        return slot2q_idx, q_idx_to_size

    # Case B: derive from groups
    groups = list(meta.get("groups", []))
    if len(groups) == 0:
        # last fallback: assume each slot has its own q
        slot2q_idx = list(range(K))
        # if codebook_sizes exist use them, else leave empty
        cb = list(meta.get("codebook_sizes", []))
        q_idx_to_size = {i: int(cb[i]) for i in range(min(K, len(cb)))} if len(cb) else {}
        return slot2q_idx, q_idx_to_size
    ...
    for g in groups:
        q = int(g["q_idx"])
        size = int(g["codebook_size"])
        q_idx_to_size[q] = size
        for sid in g["ids"]:
            sid = int(sid)
            if sid < 0 or sid >= K:
                raise ValueError(f"[meta] group slot id out of range: {sid} (K={K})")
            slot2q_idx[sid] = q

    if any(v < 0 for v in slot2q_idx):
        missing = [i for i, v in enumerate(slot2q_idx) if v < 0]
        raise ValueError(f"[meta] some slots not covered by groups: {missing}")

    return slot2q_idx, q_idx_to_size
def build_smplx_and_vertex_tensors(opt, device, *, require: bool, smplx_model_path_fallback: str = None):
    """
    Build:
      smplx_model: smplx.create(...).eval()
      vert_idx: LongTensor [Nv] (all verts indices)
      vert_w:   FloatTensor [Nv] (weights per vert; hands *10)
    Requires opt fields:
      - opt.UPPER_BODY_VERTEX, opt.LEFT_HAND_VERTEX, opt.RIGHT_HAND_VERTEX (lists of ints)
      - opt.smplx_model_path (or CONFIG fallback)

    If require=True: raise on any missing dependency.
    """
    try:
        import smplx
    except Exception as e:
        if require:
            raise RuntimeError(f"[vertex_loss] smplx import failed: {e}") from e
        print(f"[vertex_loss] smplx import failed: {e}")
        return None, None, None

    model_path = getattr(opt, "smplx_model_path", None) or getattr(opt, "SMPLX_MODEL_PATH", None)
    if model_path is None and smplx_model_path_fallback:
        model_path = smplx_model_path_fallback

    if model_path is None:
        if require:
            raise RuntimeError("[vertex_loss] Missing smplx_model_path (opt.smplx_model_path/SMPLX_MODEL_PATH or CONFIG['smplx_model_path']).")
        print("[vertex_loss] opt has no smplx_model_path; vertex loss disabled.")
        return None, None, None

    ub = list(getattr(opt, "UPPER_BODY_VERTEX", []))
    lh = list(getattr(opt, "LEFT_HAND_VERTEX", []))
    rh = list(getattr(opt, "RIGHT_HAND_VERTEX", []))

    if len(ub) == 0 or len(lh) == 0 or len(rh) == 0:
        if require:
            raise RuntimeError(
                "[vertex_loss] Missing vertex index lists on opt: need UPPER_BODY_VERTEX / LEFT_HAND_VERTEX / RIGHT_HAND_VERTEX"
            )
        print("[vertex_loss] vertex index lists missing; vertex loss disabled.")
        return None, None, None

    # build tensors
    all_idx = ub + lh + rh
    vert_idx = torch.tensor(all_idx, dtype=torch.long, device=device)

    # weights: hands *10, body *1
    w = torch.ones((len(all_idx),), dtype=torch.float32, device=device)
    n_ub = len(ub)
    w[n_ub : n_ub + len(lh)] *= 10.0
    w[n_ub + len(lh) :] *= 10.0
    vert_w = w

    # batch_size: safest to over-allocate a bit (vertex loss will sample frames anyway)
    # If you want strict: set a fixed upper bound like 4096.
    ...
    return smplx_model, vert_idx, vert_w



def _pose43_to_pose55(pose43, selected_joint_indices, device):
    """
    pose43: [N, 43, 3]  (axis-angle)
    return: [N, 55, 3]
    """
    N = pose43.shape[0]
    pose55 = torch.zeros((N, 55, 3), dtype=pose43.dtype, device=device)
    idx = torch.as_tensor(selected_joint_indices, dtype=torch.long, device=device)  # [43]
    pose55[:, idx, :] = pose43
    return pose55


def _smplx_vertices_from_pose55(smplx_model, pose55):
    """
    pose55: [N,55,3] axis-angle
    returns vertices: [N, V, 3]
    """
    N = pose55.shape[0]
    global_orient = pose55[:, 0, :]                      # [N,3]
    body_pose     = pose55[:, 1:22, :].reshape(N, -1)    # [N,63]
    lhand_pose    = pose55[:, 22:37, :].reshape(N, -1)   # [N,45]
    rhand_pose    = pose55[:, 37:52, :].reshape(N, -1)   # [N,45]
    jaw_pose      = pose55[:, 52, :]                     # [N,3]
    leye_pose     = pose55[:, 53, :]                     # [N,3]
    reye_pose     = pose55[:, 54, :]                     # [N,3]

    betas = torch.zeros((N, 10), dtype=pose55.dtype, device=pose55.device)
    expr  = torch.zeros((N, 10), dtype=pose55.dtype, device=pose55.device)
    transl = torch.zeros((N, 3), dtype=pose55.dtype, device=pose55.device)
    ...
    return out.vertices  # [N,V,3]


def _logits_to_soft_zquant(logits_dict, slot_names, qvae, slot2q_idx, soft_temp=1.0):
    """
    logits_dict[name]: [B,T,V(+2)]  (maskgit head 通常是 cb+2)
    目标：构造 z_quant_soft: [B,T,K,D]，并且对 logits 可导
    """
    device = next(iter(logits_dict.values())).device
    B, T, _ = next(iter(logits_dict.values())).shape
    K = len(slot_names)
    D = int(qvae.latent_dim)

    z = torch.zeros((B, T, K, D), dtype=next(iter(logits_dict.values())).dtype, device=device)

    # 按 qvae 的 grouping_schedule 来取 embedding weight（保证和 decode_from_tokens 一致）
    # 每组：同一个 quantizer（同一个 embedding weight）给若干 node ids 用
    for group in qvae.grouping_schedule:
        ids = list(group["ids"])
        q_idx = int(group["q_idx"])
        emb = qvae.quantizers[q_idx]._embedding.weight  # [Cb, D]
        cb = emb.shape[0]

        for node_id in ids:
            name = slot_names[node_id]
            lg = logits_dict[name]  # [B,T,V]
            # 只对真实 codeword 做 soft（排除最后 2 个 special）
            lg = lg[..., :cb] / max(float(soft_temp), 1e-6)
            prob = F.softmax(lg, dim=-1)                # [B,T,Cb]
            z[:, :, node_id, :] = prob @ emb            # [B,T,D]

    return z

def preflight_vertex_loss_or_die(
    *,
    vtx_w: float,
    qvae,
    smplx_model,
    ...
    """
    If vtx_w>0, require everything to be ready. Otherwise do nothing.
    """
    if float(vtx_w) <= 0:
        return

    missing = []
    if qvae is None: missing.append("qvae")
    if smplx_model is None: missing.append("smplx_model")
    if vert_idx is None: missing.append("vert_idx")
    if vert_w is None: missing.append("vert_w")
    if selected_joint_indices is None or len(list(selected_joint_indices)) == 0:
        missing.append("selected_joint_indices")
    if slot2q_idx is None or len(list(slot2q_idx)) == 0:
        missing.append("slot2q_idx")

    if missing:
        raise RuntimeError(
            f"[vertex_loss] vertex_loss_weight={vtx_w} but assets are missing: {missing}\n"
            f"Fix: provide SMPL-X path + vertex index lists in opt, and ensure meta provides groups/slot2q_idx."
        )


def compute_vertex_loss_failfast(
    *,
    qvae,
    smplx_model,
    vert_idx,
    ...
    """
    Wrap your existing _compute_vertex_loss but NEVER swallow exceptions.
    """
    # _compute_vertex_loss is already implemented in your train_maskgit_rag.py :contentReference[oaicite:3]{index=3}
    return _compute_vertex_loss(
        qvae=qvae,
        smplx_model=smplx_model,
        vert_idx=vert_idx,
        vert_w=vert_w,
        selected_joint_indices=selected_joint_indices,
        logits_for_all_slots=logits_for_all_slots,
        motion_gt_tokens=motion_gt_tokens,
...
def train_one_epoch(
    ...
    # ===== vertex loss config =====
    vtx_w = float(CONFIG.get("vertex_loss_weight", 0.0))
    vtx_prob = float(CONFIG.get("vertex_loss_prob", 0.25))      # 抽样算，别每步都算
    vtx_max_frames = int(CONFIG.get("vertex_loss_max_frames", 256))
    vtx_soft_temp = float(CONFIG.get("vertex_soft_temp", 1.0))
    ...
    for batch in pbar:
        ...
        # glosses: list[str], provided by dataset/pad_collate (you said you've wired it)
        
        # ---- blueprint (可选；如果你当前脚本没定义相关函数，就直接跳过) ----
        use_bp = bool(CONFIG.get("use_blueprint", False))
        bp_tokens = None
        bp_pad_mask = None
        if use_bp and (glosses is not None):
            # 这段完全“容错”：没函数/没 map 就不挂
            if ("_load_wlasl_map" in globals()) and ("build_blueprint_batch" in globals()):
                try:
                    ...
                    if n_steps % 200 == 0:
                        print(f"[blueprint] build failed, fallback to no-bp. err={repr(e)}")
        ...
        with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=(device.type == "cuda")):
            ...
            if iterative:
                ...
                if mask_map.dim() == 3:
                    mask_map2 = mask_map & (~keep_map.unsqueeze(-1))
                else:
                ...
                logits_for_vtx = logits2  # 用第二次 refinement 的 logits
            else:
                logits, reps = model(
                    x0, text,
                    key_padding_mask=key_padding_mask,
            ...
            # ===== vertex loss (可导) =====
            vtx_loss = mlm_loss.new_tensor(0.0)
            if int(CONFIG.get("vertex_loss_weight", 0.0))!=0:
                preflight_vertex_loss_or_die(
                    vtx_w=vtx_w, qvae=qvae, smplx_model=smplx_model, vert_idx=vert_idx, vert_w=vert_w,
                    selected_joint_indices=selected_joint_indices, slot2q_idx=slot2q_idx,
                )
                vtx_loss = compute_vertex_loss_failfast(
                    qvae=qvae, smplx_model=smplx_model, vert_idx=vert_idx, vert_w=vert_w,
                    selected_joint_indices=selected_joint_indices,
                    logits_for_all_slots=logits,  # 你当前变量名如果不是 logits，就换成你的
                    motion_gt_tokens=motion_gt,
                    motion_pad_mask=motion_pad_mask,
                    slot_names=slot_names,
                    slot2q_idx=slot2q_idx,
                    soft_temp=float(CONFIG.get("vertex_soft_temp", 1.0)),
                    max_frames=int(CONFIG.get("vertex_loss_max_frames", 128)),
                )
            loss = mlm_loss + infonce_w * rank_loss + len_w * len_loss + vtx_w * vtx_loss
        ...
        running_vtx += float(vtx_loss.item()) if vtx_w > 0 else 0.0
        n_steps += 1
        ...
        if n_steps % 50 == 0:
            pbar.set_postfix({
                ...
                "vtx": f"{vtx_loss.item():.4f}" if vtx_w > 0 else "off",
    ...
    hparams = {
        ...
        "vertex_loss_weight": float(vtx_w),
        "vertex_loss_prob": float(vtx_prob),
        "vertex_loss_max_frames": int(vtx_max_frames),
        "vertex_soft_temp": float(vtx_soft_temp),
    }
...
def _load_simple_opt_from_txt(config_path: str, device):
    """
    从 opt.txt 读 argparse.Namespace，并做最小必要的类型转换。
    """
    import argparse
...
def load_qvae_and_opt(config_path: str, model_path: str, device):
    """
    ✅ 统一入口：加载 opt + QVAE
    返回: (qvae, opt)

    - 会自动把 SELECTED_JOINT_INDICES / NEIGHBOR_LIST patch 进 opt
    - 兼容 ckpt 格式: {'vae':...} / {'model':...} / 直接 state_dict
    """
    import torch
    from mymodel.vae.qvae_model_rod3_fixed_length import VAE

    print(f"🎨 Loading QVAE (shared)...")
    print(f"   Config: {config_path}")
    print(f"   Model:  {model_path}")

    opt = _load_simple_opt_from_txt(config_path, device)
    ...
    if isinstance(ckpt, dict):
        if "vae" in ckpt:
            qvae.load_state_dict(ckpt["vae"])
        elif "model" in ckpt:
            qvae.load_state_dict(ckpt["model"])
        else:
...
def load_qvae_for_vis(config_path, model_path, device):
    ...
    只返回 qvae
    """
    qvae, _ = load_qvae_and_opt(config_path, model_path, device)
    return qvae
@torch.no_grad()
def visualize_and_save(maskgit_model, val_loader, config, device, qvae=None, opt=None, meta=None, pad_token_ids=None):
    """
    Run generation and save as full SMPL-X format npz.

    ✅ 优先复用外部传入的:
      - qvae / opt（避免重复加载）
      - meta / pad_token_ids（避免重复 load_metadata）

    Compatible batch:
      - non-RAG: (motion_gt, text_emb, lengths, motion_pad_mask, text_pad_mask, names)
      - RAG:     (motion_gt, text_emb, lengths, motion_pad_mask, text_pad_mask, names, glosses)
    """
    print("\n🎬 Starting Visualization Generation (Full SMPL-X format)...")

    # 1) Prepare QVAE (reuse if provided)
    if qvae is None or opt is None:
        ...
        if not qvae_cfg or not qvae_pth:
            ...
            return
        qvae, opt = load_qvae_and_opt(qvae_cfg, qvae_pth, device)
    ...
    # 4) Optional: build blueprint (only if glosses exist)
    bp_tokens = None
    bp_pad_mask = None
    if glosses is not None:
        try:
            # meta/pad_token_ids 复用
            if meta is None:
            ...
            print(f"[visualize_and_save] ⚠️ build blueprint failed, fallback to non-RAG vis. err={repr(e)}")
            bp_tokens = None
            bp_pad_mask = None

    # 5) Take top 3
    num_samples = min(3, motion_gt.shape[0])
    # 你现在 run_vis_from_ckpt 里传 out_dir，所以这里优先用 out_dir；否则用 save_dir/vis_results
    base_out = str(config.get("out_dir", "")).strip()
    if base_out:
        results_dir = os.path.join(base_out, "vis_results")
    else:
        results_dir = os.path.join(config["save_dir"], "vis_results")
    os.makedirs(results_dir, exist_ok=True)

    def _call_generate(model, **kwargs):
        # 兼容两种 model.generate 签名（带/不带 blueprint）
        try:
...
def main():
    ...
    # ---- meta -> slot2q/q_idx_to_size (always available) ----
    slot2q_idx, q_idx_to_size = build_slot2q_and_sizes_from_meta(meta)
    # ---- model ----
    model = MaskGITTransformer(
        ...
        tie_groups=bool(CONFIG.get("tie_groups", False)),
        flatten_spatiotemporal=True,
        use_blueprint=bool(CONFIG.get("use_blueprint", False)),
    ).to(device)
    ...
    # ---- vertex assets (ONLY ONCE) ----
    vtx_w = float(CONFIG.get("vertex_loss_weight", 0.0))
    qvae, qopt = (None, None)
    smplx_model, vert_idx, vert_w = (None, None, None)
    selected_joint_indices = None

    if vtx_w > 0:
        qvae, qopt = load_qvae_and_opt(CONFIG["qvae_config_path"], CONFIG["qvae_model_path"], device)  # 你现有的
        qvae.eval()
        for p in qvae.parameters():
            p.requires_grad_(False)

        selected_joint_indices = list(getattr(qopt, "SELECTED_JOINT_INDICES", []))
        smplx_model, vert_idx, vert_w = build_smplx_and_vertex_tensors(
            qopt, device,
            require=True,
            smplx_model_path_fallback=str(CONFIG.get("smplx_model_path", "")).strip() or None,
        )
    ...
    for epoch in range(start_epoch, int(CONFIG["epochs"])):
        ...
        train_loss, train_acc, train_rank, train_len_loss, hparams = train_one_epoch(
            ...
            vert_w=vert_w,
            selected_joint_indices=selected_joint_indices,
            slot2q_idx=slot2q_idx,   # 你 metadata 里那个 list，长度=K
        )
